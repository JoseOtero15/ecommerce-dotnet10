...
namespace ApiEcommerce.Controllers
{
  ...
  public class ProductsController : ControllerBase
  {
    ...
    public IActionResult GetProductsForCategory(int categoryId)
    {
      ...
      if (products.Count == 0) return NotFound($"Los productos con la categoría {categoryId} no existen");
      var productsDto = _mapper.Map<List<ProductDto>>(products);
    ...
    public IActionResult SearchProducts(string searchTerm)
    {
      ...
      if (products.Count == 0) return NotFound($"Los productos con el nombre o descripción '{searchTerm}' no existen");
    ...
    {
      if (string.IsNullOrWhiteSpace(name) || quantity <= 0) return BadRequest("El nombre del producto o la cantidad no son válidos");
